//
//  ProfileViedionewViewController.swift
//  GulfExchangeApp
//
//  Created by test on 25/10/21.
...
class ProfileViedionewViewController: UIViewController {
       ...
       /// ViedionewViewController.startRecord(delegate: self, sourceType: .camera)
        // ViedionewViewController.startRecord(delegate: self, sourceType: .camera)
         ...
         let showAlert = UIAlertController(title: "", message: nil, preferredStyle: .alert)
                      ...
                      let label = UILabel(frame: CGRect(x: 15, y: 255, width: 260, height: 40))
                      label.text = "Capture a 3-second video of yourself blinking while holding your ID."
...
//    }
    ...
    @IBAction func openImgVideoPicker() {
        ...
        if ((defaults.string(forKey: "strBase64videoProfile")?.isEmpty) != nil)
        {
...
extension ProfileViedionewViewController : UIImagePickerControllerDelegate , UINavigationControllerDelegate
{
...
//        present(alert1, animated: true, completion: nil)
        ...
        if let videoData = NSData(contentsOf: url) {
            ...
            self.strBase64videoProfile = videoData.base64EncodedString(options: .lineLength64Characters)
            
            self.defaults.set(strBase64videoProfile, forKey: "strBase64videoProfile")
...
//            present(alert1, animated: true, completion: nil)
           ...
           // let alert = UIAlertController(title: "Success", message: "Video was saved", preferredStyle: .alert)
            
           // alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { [self] action in
                
                self.selfielabelphotostr = " Selfie with ID selected"
                self.defaults.set(self.selfielabelphotostr, forKey: "selfielabelphotostr")
                //nextvc
...
// DOESN'T CRASH
@objc protocol MyDelegatee: UIImagePickerControllerDelegate {

}

extension MyDelegatee {
