//
//  RemittancePage2ViewController.swift
//  GulfExchangeApp
//
...
class RemittancePage2ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource, UITextFieldDelegate {
    ...
    @IBAction func sampleswitchAct(_ sender: Any)
    {
        ...
        if sampleswitch.isOn {
             print("ONnow")
                 ...
                 let alertControllerotp = UIAlertController(title: "Loyalty Code", message: "", preferredStyle: .alert)
                 ...
                 let saveAction = UIAlertAction(title: "Ok", style: .default, handler: { alert -> Void in
                     ...
                     firstTextField.placeholder = "Enter Loyalty Code"
                     ...
                     print(strotp)
                     // "this is the answer"
                     print("strpromo",strotp)
                     firstTextField.text =  strotp
                     print("promotxtfiled.text",firstTextField.text)
                     ...
                     guard let mobile = firstTextField.text,firstTextField.text?.count != 0 else
                     {
                         ...
                         self.showToast(message: "Enter Loyalty Code", font: .systemFont(ofSize: 12.0))
                    ...
                    // userdefault store
                    UserDefaults.standard.removeObject(forKey: "promocodee")
                    self.defaults.set(firstTextField.text!, forKey: "promocodee")
                 ...
                 let cancelAction = UIAlertAction(title: "Cancel", style: .default, handler: { alert -> Void in
                     //self.getToken(num: 7)
                    self.sampleswitch.isOn = false
                    UserDefaults.standard.removeObject(forKey: "promocodee")
                    self.defaults.set("-", forKey: "promocodee")
                    self.dismiss(animated: true, completion: nil)
                     
                 })
                 alertControllerotp.addTextField { (textField : UITextField!) -> Void in
                     textField.placeholder = "Enter Loyalty Code"
...
//            self.scrollView.setContentOffset(bottomOffset, animated: true)
    ...
    //new
    var reciveamountFtyperespstr:String = ""
...
//            }()
  ...
  //new
        UserDefaults.standard.removeObject(forKey: "promocodee")
        self.defaults.set("-", forKey: "promocodee")
        self.promoLbl.text = "Loyalty Code"
        self.promoCodeLbl.text = "Do you have a Loyalty Code?"
...
//    }
    ...
    @objc func Backbtnaction()
    {
        ...
        print("servicetypestoredstr2iiiii",servicetypestoredstr)
        //crashing
...
//                self.scrollView.setContentOffset(bottomOffset, animated: true)
                ...
                self.view.makeToast("Select convert", duration: 3.0, position: .center)
...
//            self.scrollView.setContentOffset(bottomOffset, animated: true)
    ...
    func getToken(num:Int) {
        ...
        RemittancePage2ViewController.AlamoFireManager.request(url, method: .post, parameters: nil, encoding: JSONEncoding.default, headers: headers).responseJSON(completionHandler: { (response) in
    ...
    func amountValidation(access_token:String) {
        ...
        let params:Parameters =  ["partnerId":partnerId,"token":token,"requestTime":dateTime,"customerIDNo":self.defaults.string(forKey: "USERID")!,"customerPassword":"","mpin":"","customerEmail":"","customerMobile":"","customerPhone":"","beneficiaryAccountNo":self.defaults.string(forKey: "serial_no")!,"iban":"","customerDOB":"1900-01-01","validationMethod":"PAYIN_AMOUNT","isExistOrValid":self.sendAmountTextField.text!]
        ...
        RemittancePage2ViewController.AlamoFireManager.request(url, method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers).responseJSON(completionHandler: { (response) in
            let myResult = try? JSON(data: response.data!)
            self.effectView.removeFromSuperview()
            print("resp",response)
            ...
            if(respCode == "E8009")
            {
                ...
                else if(Double(self.sendAmountTextField.text!) == 0)
                {
                    ...
                    else
                   {
                    ...
                    self.defaults.set(self.sendAmountTextField.text!, forKey: "amount1")
                    self.defaults.set(self.receivingAmountTextField.text!, forKey: "amount2")
                   }
        ...
        RemittancePage2ViewController.AlamoFireManager.request(url, method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers).responseJSON(completionHandler: { (response) in
            ...
            for i in resultArray.arrayValue{
               ...
               i["currencyCode"].stringValue
                ...
                self.singlecontry2codearrtwo.append(i["countrycode"].stringValue)
                
                
                let bank = Bank(bankAddress: i["currencyCode"].stringValue, bankCode: i["bankCode"].stringValue, bankName: i["currencyName"].stringValue, bankcitytype: i["countrycode"].stringValue,Accountnolength: i["requiredBranch"].stringValue,Mobnolength: i["recMobLength"].stringValue,ifscnolength: i["recMobLength"].stringValue,Minaccnolength: i["recMobLength"].stringValue)
...
func feelookup(amount:String,receCountry:String,receCurrency:String,from:String,access_token:String) {
    ...
    //branch code checdk
        ...
        let params:Parameters =  ["agentId":agentId,"token":token_remi,"timeStamp":dateTime,"sendCountry":"QAT","sendCurrency":"QAR","receiveCountry":receCountry,"receiveCurrency":receCurrency,"deliveryOption":servicetypestoredstr,"providerId":"1","sendAmount":amount,"serviceProviderCode":bankcodeserviceprovidercodestr,"cityCode":citycodestr,"payoutBranchCode":branchcodestr,"customerIDNo":defaults.string(forKey: "USERID")!,"deviceType":"IOS","versionName":appVersion,"types":"M","amountType":"L"]
...
func feelookupsecondtxtnew(amount:String,receCountry:String,receCurrency:String,from:String,access_token:String) {
    ...
    let params:Parameters =  ["agentId":agentId,"token":token_remi,"timeStamp":dateTime,"sendCountry":"QAT","sendCurrency":"QAR","receiveCountry":receCountry,"receiveCurrency":receCurrency,"deliveryOption":servicetypestoredstr,"providerId":"1","sendAmount":receivingAmountTextField.text!,"serviceProviderCode":bankcodeserviceprovidercodestr,"cityCode":citycodestr,"payoutBranchCode":branchcodestr,"customerIDNo":defaults.string(forKey: "USERID")!,"deviceType":"IOS","versionName":appVersion,"types":"M","amountType":"F"]
...
//                let formattedNumber = numberFormatter.string(from: NSNumber(value:rec_amount!))
                ...
                if(from == "qar_to_any")
                {
                    ...
                    self.rateLbl.text = "1 QAR = " + self.retailExchangeRate + " " + receCurrency
                    self.transferAmountLbl1.text = self.sendAmountTextField.text! + " QAR"
                    if(self.fee_amount1 == "0.0")
                    {
                        self.transferFeeLbl1.text = self.fee_amount1 + "0 QAR"
                    }
                    ...
                    let amount1 = Double(self.sendAmountTextField.text!) ?? 0.0
                    let amount2 = Double(self.fee_amount1) ?? 0.0
...
//                    self.totalglobdisplay = String(format: "%.2f", totalglobdisplayy as! CVarArg)
                  ...
                  self.defaults.set(self.retailExchangeRate1, forKey: "exchangeRate")
                    ...
                    //new
                       self.reciveamountFtyperespstr = myResult!["receiveAmount"].stringValue
                    self.receivingAmountTextField.text = self.reciveamountFtyperespstr
                }
            }
            else{
                if(from == "qar_to_any")
    ...
    func feelookupsecondtxtnenextbtn(amount:String,receCountry:String,receCurrency:String,from:String,access_token:String) {
           ...
           //let servicetypestored = defaults.string(forKey: "servicetypestoresel")!
           // print("servicetypestored:\(servicetypestored)")
        print("feelookup","feelooksecondnext")
            //servicetypestored check
        let userdefaultsservicetype = UserDefaults.standard
        if let savedValue = userdefaultsservicetype.string(forKey: "servicetypestoresel"){
            print("Here you will get saved value")
