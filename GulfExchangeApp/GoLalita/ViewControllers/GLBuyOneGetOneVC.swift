...
class GLBuyOneGetOneVC: UIViewController {
    ...
    @IBOutlet weak var applyOfferBtn: UIButton!
    ...
    @IBAction func applyOfferBtnTapped(_ sender: Any) {
        self.navigationItem.backBarButtonItem = UIBarButtonItem(title: "", style: .plain, target: nil, action: nil)
        let vc: GLBuyOneCodeVC = UIStoryboard.init(name: "Dummy", bundle: Bundle.main).instantiateViewController(withIdentifier: "GLBuyOneCodeVC") as! GLBuyOneCodeVC
        vc.merchantName = pageDetails?.merchantName ?? "Merchant"
        vc.offerTitle = pageDetails?.descriptionSale ?? ""
        vc.productID = pageDetails?.id ?? 0
...
extension GLBuyOneGetOneVC: GolalitaSideMenuDelegate{
